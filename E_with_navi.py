...
<head>
    ...
    <script>
        ...
        function toggleCollapse(sectionId) {
            var allSections = document.getElementsByClassName('table-container'); // Get all collapsible sections
            ...
            element.style.display = 'table';  // Expand the selected course
        }
    </script>
</head>
<body>
...
for course, criterion in kb.criterionE.items():
    course_id = course.replace(" ", "_")  # Generate a unique ID for each course
    ...
    if not filtered_criterion_df.empty or not filtered_criterion_qa_df.empty:
        # Add course name with collapsible accordion
        html_content += f'<div class="course-name" id="{course_id}" onclick="toggleCollapse(\'table_{course_id}\')">{course}</div>'
        
        # Add Criterion header
        html_content += '<div class="criterion-header">Criterion E: Integrated and Applied ICT Knowledge</div>'
        
        # Start collapsible table container
        html_content += f"""
        <div class="table-container" id="table_{course_id}" style="display:none">
        <table>
            <thead>
                <tr>
...
output_html_path = 'E_navigable.html'
